...
public interface ReservationMapper extends BaseMapper<Reservation> {
    ...
    // 查找所有待审批的预约列表，返回列表包含预约信息、预约用户名称和会议室名称，按预约开始时间升序排序
    @Select("SELECT r.*, u.name AS userName, m.room_name AS roomName FROM t_reservation r JOIN t_user u ON r.user_id = u.id JOIN t_meeting_room m ON r.room_id = m.id WHERE r.reservation_status = 0 ORDER BY r.start_time ASC")
