...
public interface ReservationMapper extends BaseMapper<Reservation> {
    ...
    // 查找指定用户的预约列表（分页 + 状态筛选），返回列表包含预约信息、会议室名称和签到状态，按预约开始时间降序排序。
    // status 语义：null=全部；0=待审批；1=已通过；2=已结束（已拒绝/被覆盖/已取消/超时释放）
    @Select("<script>" +
            "SELECT r.*, m.room_name AS roomName, ar.attend_status " +
            "FROM t_reservation r JOIN t_meeting_room m ON r.room_id = m.id " +
            "LEFT JOIN t_attendance_record ar ON r.id = ar.reservation_id " +
            "WHERE r.user_id = #{userId} " +
            "<choose>" +
            "<when test='status == 2'>AND r.reservation_status IN (2, 3, 4, 5)</when>" +
            "<when test='status != null'>AND r.reservation_status = #{status}</when>" +
            "</choose>" +
            "ORDER BY r.start_time DESC" +
            "</script>")
    IPage<Map<String, Object>> findByUserIdWithRoomPage(Page<?> page,
