...
function dayRange() {
    var d = getDate();
    return [new Date(d + 'T08:00:00').getTime(), new Date(d + 'T22:00:00').getTime()];
}
...
function refreshChart() {
    ...
    if (selectedStart && selectedEnd) {
        ...
        data.push({ value: [1, selectedStart.getTime(), selectedEnd.getTime(), '#198754', ''] });
    } else if (firstClickTime) {
        // 用户只点了第一次（起点）：画一个 15 分钟的黄色提示条
        // 15*60000 = 15分钟 = 900000毫秒，表示"暂时假设你选的是起始点+15分钟"
        data.push({ value: [1, firstClickTime.getTime(), firstClickTime.getTime() + 15 * 60000, '#ffc107', ''] });
    }
    ...
    chart.setOption({
        tooltip: {
            ...
            formatter: function (p) {
                ...
                var s = new Date(v[1]), e = new Date(v[2]);
                var label = pad(s.getHours()) + ':' + pad(s.getMinutes()) + ' ~ ' +
...
function loadSchedule() {
    ...
    axios.get('/api/room-schedule?roomId=' + roomId + '&date=' + getDate())
        .then(function (res) {
...
chart.getZr().on('click', function (e) {
    ...
    if (clickCount === 0) {
        ...
        var hasStudentConflict = currentUserRole !== 'TEACHER' && hasStudentOverlap(a, b);
...
// 1. 将日期选择器设为今天（时区无关，取浏览器当地日期的 YYYY-MM-DD）
